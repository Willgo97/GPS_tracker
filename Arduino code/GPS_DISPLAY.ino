...
{ //  Calculates the battery voltage, and calculates the battery bar offset accordingly.
  double bat_voltage = (analogRead(A3) * (3.3 / 1023.00)); //read the A0 pin value
...
{ //  checks if the user pressed the button.
  ...
  else if ( digitalRead(3) == HIGH && reset_count == 3 )
  {// Reset the records when the user pressed the button for > 3 sec.
    reset_count = 0;
    count = 0;
    max_speed = 0;
    avg_speed = 0;    
...
{ 
  gpsPort.begin( 9600 );
  //  Should set the i2c speed to 400kbit/sec instead of 100kbit/sec? No idea if it works...
  TWBR = 12;
}
//------------------------------------------------------------------------------------------------
void loop()
{ //  Check if the Gps is sending data. Parsing takes about 125ms
  while (gps.available( gpsPort )) {
    //  First the gps data get's parsed into the fix object.
    fix = gps.read();
    //  Do all the calculations in the quiet time, when the GPS isn't sending any data.
    calc_data();
    //  Draw the information onto the oled display.
    u8g.firstPage();
    do {
      draw();
    } while ( u8g.nextPage() );
